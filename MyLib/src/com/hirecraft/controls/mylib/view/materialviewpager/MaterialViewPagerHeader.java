...
/**
 * Created by florentchampigny on 25/04/15. A class containing references to
 * views inside MaterialViewPager's header
 */
public class MaterialViewPagerHeader {
	...
	public MaterialViewPagerHeader withPagerSlidingTabStrip(
		...
		mPagerSlidingTabStrip.getViewTreeObserver().addOnPreDrawListener(
	...
	public MaterialViewPagerHeader withToolbarLayoutBackground(
			View toolbarLayoutBackground) {
	...
	public MaterialViewPagerHeader withLogo(View logo) {
		...
		mLogo.getViewTreeObserver().addOnPreDrawListener(
				new ViewTreeObserver.OnPreDrawListener() {
					@Override
					public boolean onPreDraw() {
						...
						// (mLogo.getWidth()/2) *(1-finalScale) is the margin
						// left added by the scale() on the logo
						// when logo scaledown, the content stay in center, so
						// we have to anually remove the left padding
						finalTitleX = dpToPx(52f, context)
								- (mLogo.getWidth() / 2) * (1 - finalScale);

						mLogo.getViewTreeObserver().removeOnPreDrawListener(
								this);
						return false;
					}
				});
