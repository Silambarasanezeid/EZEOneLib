...
import com.hirecraft.controls.mylib.view.pagerslidingtabstrip.ObservableScrollView;
import com.hirecraft.controls.mylib.view.pagerslidingtabstrip.ObservableScrollViewCallbacks;
import com.hirecraft.controls.mylib.view.pagerslidingtabstrip.ObservableWebView;
import com.hirecraft.controls.mylib.view.pagerslidingtabstrip.ScrollState;
...
import static com.hirecraft.controls.mylib.view.materialviewpager.Utils.colorWithAlpha;
import static com.hirecraft.controls.mylib.view.materialviewpager.Utils.dpToPx;
import static com.hirecraft.controls.mylib.view.materialviewpager.Utils.minMax;
import static com.hirecraft.controls.mylib.view.materialviewpager.Utils.setBackgroundColor;
import static com.hirecraft.controls.mylib.view.materialviewpager.Utils.setElevation;
import static com.hirecraft.controls.mylib.view.materialviewpager.Utils.setScale;

/**
 * Created by florentchampigny on 24/04/15.
 * <p/>
 * Listen to Scrollable inside MaterialViewPager When notified scroll, dispatch
 * the current scroll to other scrollable
 * <p/>
 * Note : didn't want to translate the MaterialViewPager or intercept Scroll, so
 * added a ViewPager with scrollables containing a transparent placeholder on
 * top
 * <p/>
...
public class MaterialViewPagerAnimator {
	...
	private static final String TAG = MaterialViewPagerAnimator.class
			.getSimpleName();
	...
	// final toolbar layout elevation (if attr viewpager_enableToolbarElevation
	// = true)
	...
	public MaterialViewPagerAnimator(MaterialViewPager materialViewPager) {
		...
		// initialise the scrollMax to headerHeight, so until the first cell
		...
		// heightMaxScrollToolbar =
		// context.getResources().getDimension(R.dimen.material_viewpager_padding_top);
	...
	private void setScrollOffset(Object scroll, float yOffset) {
		...
		if (scroll != null && yOffset >= 0) {
			...
			if (scroll instanceof RecyclerView) {
				...
				// Have to be instanceOf RecyclerView.LayoutManager to work (so
				// work with RecyclerView.GridLayoutManager)
				RecyclerView.LayoutManager layoutManager = ((RecyclerView) scroll)
						.getLayoutManager();
				if (layoutManager instanceof LinearLayoutManager) {
					...
					staggeredGridLayoutManager.scrollToPositionWithOffset(0,
							(int) -yOffset);
				}
			...
			// save the current yOffset of the scrollable on the yOffsets
			// hashmap
	...
	/**
	 * Called when a scroller(RecyclerView/ListView,ScrollView,WebView) scrolled
	 * by the user
	 * 
	 * @param source
	 *            the scroller
	 * @param yOffset
	 *            the scroller current yOffset
	 */
	public void onMaterialScrolled(Object source, float yOffset) {
...

		{
			...
			if (mHeader.headerBackground != null) {
				...
				if (this.settings.parallaxHeaderFactor != 0)
					ViewHelper.setTranslationY(mHeader.headerBackground,
		...
		percent = minMax(0, percent, 1);
		{
			...
			{
				// change color of toolbar & viewpager indicator &
				// statusBaground
				setColorPercent(percent);
				lastPercent = percent; // save the percent
			}

			if (mHeader.mPagerSlidingTabStrip != null) { // move the viewpager
															// indicator
				// float newY = ViewHelper.getY(mHeader.mPagerSlidingTabStrip) +
				// scrollTop;
				...
				if (scrollTop <= 0) {
					ViewHelper.setTranslationY(mHeader.mPagerSlidingTabStrip,
							scrollTop);
					ViewHelper.setTranslationY(mHeader.toolbarLayoutBackground,
							scrollTop);

					if (ViewHelper.getY(mHeader.mPagerSlidingTabStrip) < mHeader
							.getToolbar().getBottom()) {
						float ty = mHeader.getToolbar().getBottom()
								- mHeader.mPagerSlidingTabStrip.getTop();
						ViewHelper.setTranslationY(
								mHeader.mPagerSlidingTabStrip, ty);
						ViewHelper.setTranslationY(
								mHeader.toolbarLayoutBackground, ty);
					}
				}
			...
			if (mHeader.mLogo != null) { // move the header logo to toolbar
				...
				if (this.settings.hideLogoWithFade) {
					...
					ViewHelper.setTranslationY(mHeader.mLogo,
							(mHeader.finalTitleY - mHeader.originalTitleY)
									* percent);
				} else {
					ViewHelper.setTranslationY(mHeader.mLogo,
							(mHeader.finalTitleY - mHeader.originalTitleY)
									* percent);
					ViewHelper.setTranslationX(mHeader.mLogo,
							(mHeader.finalTitleX - mHeader.originalTitleX)
									* percent);

					float scale = (1 - percent) * (1 - mHeader.finalScale)
							+ mHeader.finalScale;
			...
			if (this.settings.hideToolbarAndTitle
					&& mHeader.toolbarLayout != null) {
				...
				if (scrollUp) {
					...
					if (yOffset > mHeader.toolbarLayout.getHeight()) {
						...
						if (headerAnimator != null) {
							ViewHelper
									.setTranslationY(mHeader.toolbarLayout, 0);
	...
	/**
	 * Change the color of the statusbackground, toolbar, toolbarlayout and
	 * pagertitlestrip With a color transition animation
	 * 
	 * @param color
	 *            the final color
	 * @param duration
	 *            the transition color animation duration
	 */
	public void setColor(int color, int duration) {
		ValueAnimator colorAnim = ObjectAnimator.ofInt(
				mHeader.headerBackground, "backgroundColor", settings.color,
				color);
		...
		colorAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				int colorAlpha = colorWithAlpha(
						(Integer) animation.getAnimatedValue(), lastPercent);
	...
	public void setColorPercent(float percent) {
		...
		setBackgroundColor(colorWithAlpha(this.settings.color, percent),
				mHeader.statusBackground);

		if (percent >= 1) {
			setBackgroundColor(colorWithAlpha(this.settings.color, percent),
					mHeader.toolbar, mHeader.toolbarLayoutBackground,
					mHeader.mPagerSlidingTabStrip);
		} else {
			setBackgroundColor(colorWithAlpha(this.settings.color, 0),
					mHeader.toolbar, mHeader.toolbarLayoutBackground,
					mHeader.mPagerSlidingTabStrip);
		}

		if (this.settings.enableToolbarElevation)
			setElevation((percent == 1) ? elevation : 0, mHeader.toolbar,
	...
	/**
	 * move the toolbarlayout (containing toolbar & tabs) following the current
	 * scroll
	 */
	private void followScrollToolbarLayout(float yOffset) {
		...
		if (diffOffsetScrollMax <= 0) {
			ViewHelper.setTranslationY(mHeader.toolbarLayout,
					diffOffsetScrollMax);
		}
	...
	private void animateEnterToolbarLayout(float yOffset) {
		...
		if (headerAnimator == null) {
			if (android.os.Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {
				headerAnimator = android.animation.ObjectAnimator.ofFloat(
						mHeader.toolbarLayout, "translationY", 0).setDuration(
						ENTER_TOOLBAR_ANIMATION_DURATION);
				((android.animation.ObjectAnimator) headerAnimator)
				...
				headerAnimator = ObjectAnimator.ofFloat(mHeader.toolbarLayout,
						"translationY", 0).setDuration(
						ENTER_TOOLBAR_ANIMATION_DURATION);
	...
	/**
	 * Register a RecyclerView to the current MaterialViewPagerAnimator Listen
	 * to RecyclerView.OnScrollListener so give to $[onScrollListener] your
	 * RecyclerView.OnScrollListener if you already use one For loadmore or
	 * anything else
	 * 
	 * @param recyclerView
	 *            the scrollable
	...
	public void registerRecyclerView(final RecyclerView recyclerView,
		...
		if (recyclerView != null) {
			...
			yOffsets.put(recyclerView, recyclerView.getScrollY()); // save the
																	// initial
																	// recyclerview's
																	// yOffset
																	// (0) into
			...
			recyclerView
					.setOnScrollListener(new RecyclerView.OnScrollListener() {
						...
						public void onScrolled(RecyclerView recyclerView,
							...
							if (onScrollListener != null)
								onScrollListener.onScrolled(recyclerView, dx,
										dy);

							int yOffset = yOffsets.get(recyclerView);

							yOffset += dy;
							yOffsets.put(recyclerView, yOffset); // save the new
																	// offset

							// first time you get 0, don't share it to others
							// scrolls
	 ...
	 * ObservableScrollViewCallbacks so give to $[observableScrollViewCallbacks]
	 * your ObservableScrollViewCallbacks if you already use one For loadmore or
	 * anything else
	 * 
	 * @param scrollView
	 *            the scrollable
	 * @param observableScrollViewCallbacks
	 *            use it if you want to get a callback of the RecyclerView
	 */
	public void registerScrollView(final ObservableScrollView scrollView,
		...
		if (scrollView != null) {
			...
			if (scrollView.getParent() != null
					&& scrollView.getParent().getParent() != null
					&& scrollView.getParent().getParent() instanceof ViewGroup)
				scrollView.setTouchInterceptionViewGroup((ViewGroup) scrollView
						.getParent().getParent());
			scrollView
					.setScrollViewCallbacks(new ObservableScrollViewCallbacks() {
						...
						@Override
						public void onScrollChanged(int yOffset, boolean b,
								boolean b2) {
						...
						public void onUpOrCancelMotionEvent(
							...
							if (observableScrollViewCallbacks != null)
								observableScrollViewCallbacks
										.onUpOrCancelMotionEvent(scrollState);
						}
					});
		...
		if (webView != null) {
			...
			webView.setScrollViewCallbacks(new ObservableScrollViewCallbacks() {
				...
				public void onScrollChanged(int yOffset, boolean b, boolean b2) {
					if (observableScrollViewCallbacks != null)
						observableScrollViewCallbacks.onScrollChanged(yOffset,
								b, b2);
