...
public class Transition {
	...
	/**
	 * Gets the rect that will take the scene when a Ken Burns transition
	 * starts.
	 * 
	...
	public RectF getInterpolatedRect(long elapsedTime) {
		...
		float interpolation = mInterpolator
				.getInterpolation(interpolationProgress);
		float currentWidth = mSrcRect.width() + (interpolation * mWidthDiff);
		float currentHeight = mSrcRect.height() + (interpolation * mHeightDiff);

		float currentCenterX = mSrcRect.centerX()
				+ (interpolation * mCenterXDiff);
		float currentCenterY = mSrcRect.centerY()
				+ (interpolation * mCenterYDiff);
