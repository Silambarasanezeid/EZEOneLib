package com.hirecraft.controls.mylib.view.pagerslidingtabstrip;
...
public class KenBurnsView extends ImageView {
	...
	/**
	 * The {@link TransitionGenerator} implementation used to perform the
	 * transitions between rects. The default {@link TransitionGenerator} is
	 * {@link RandomTransitionGenerator}.
	 */
	private TransitionGenerator mTransGen = new RandomTransitionGenerator();

	/**
	 * A {@link KenBurnsView.TransitionListener} to be notified when a
	...
	/**
	 * The time, in milliseconds, of the last animation frame. This is useful to
	 * increment {@link #mElapsedTime} regardless of the amount of time the
	...
	/**
	 * Indicates whether the parent constructor was already called. This is
	 * needed to distinguish if the image is being set before or after the super
	...
	public void setVisibility(int visibility) {
		...
		/*
		 * When not visible, onDraw() doesn't get called, but the time elapses
		 * anyway.
		 */
		switch (visibility) {
		case VISIBLE:
	...
	protected void onDraw(Canvas canvas) {
		...
		if (!mPaused && d != null) {
			if (mDrawableRect.isEmpty()) {
				...
				if (mCurrentTrans.getDestinyRect() != null) { // If null, it's
																// supposed to
																// stop.
					mElapsedTime += System.currentTimeMillis() - mLastFrameTime;
					RectF currentRect = mCurrentTrans
							.getInterpolatedRect(mElapsedTime);

					float widthScale = mDrawableRect.width()
							/ currentRect.width();
					float heightScale = mDrawableRect.height()
							/ currentRect.height();
					// Scale to make the current rect match the smallest
					// drawable dimension.
					float currRectToDrwScale = Math
							.min(widthScale, heightScale);
					// Scale to make the current rect match the viewport bounds.
					float currRectToVpScale = mViewportRect.width()
							/ currentRect.width();
					// Combines the two scales to fill the viewport with the
					// current rect.
					float totalScale = currRectToDrwScale * currRectToVpScale;

					float translX = totalScale
							* (mDrawableRect.centerX() - currentRect.left);
					float translY = totalScale
							* (mDrawableRect.centerY() - currentRect.top);

					/*
					 * Performs matrix transformations to fit the content of the
					 * current rect into the entire view.
					 */
					mMatrix.reset();
					mMatrix.postTranslate(-mDrawableRect.width() / 2,
	...
	private void startNewTransition() {
		...
		mCurrentTrans = mTransGen.generateNextTransition(mDrawableRect,
				mViewportRect);
		mElapsedTime = 0;
		mLastFrameTime = System.currentTimeMillis();
	 ...
	 * 
	 * @param transition
	 *            the transition that just started.
	 */
	 ...
	 * 
	 * @param transition
	 *            the transition that just ended.
	 */
	 ...
	 * 
	 * @param transgen
	 *            the {@link TransitionGenerator} to be used in animations.
	 */
	...
	/**
	 * Updates the viewport rect. This must be called every time the size of
	 * this view changes.
	 * 
	 * @param width
	 *            the new viewport with.
	 * @param height
	...
	/**
	 * Updates the drawable bounds rect. This must be called every time the
	 * drawable associated to this view changes.
	 */
	private void updateDrawableBounds() {
		...
		if (d != null && d.getIntrinsicHeight() > 0
			...
			mDrawableRect.set(0, 0, d.getIntrinsicWidth(),
	...
	private void handleImageChange() {
		updateDrawableBounds();
		/*
		 * Don't start a new transition if this event was fired during the super
		 * constructor execution. The view won't be ready at this time. Also,
	...
	/**
	 * A transition listener receives notifications when a transition starts or
	 * ends.
	 */
