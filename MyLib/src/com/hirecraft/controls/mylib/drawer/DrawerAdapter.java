...
public class DrawerAdapter extends ArrayAdapter<DrawerItem> {
		...
		if (drawerItem.isDivider()) {
			if (convertView == null || convertView instanceof RelativeLayout) {
				convertView = LayoutInflater.from(getContext()).inflate(
						R.layout.drawer_divider_item, parent, false);
			}
		} else {
			if (convertView == null || !(convertView instanceof RelativeLayout)) {
				convertView = LayoutInflater.from(getContext()).inflate(
			...
			if (position == selectedPosition) {
				...
				if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
					getContext().getTheme().resolveAttribute(
							android.R.attr.colorPrimary, a, true);
			...
			if (drawerItem.hasImage()) {
				...
				if (drawerItem.getImageMode() == DrawerItem.AVATAR) {
					imageSize = getContext().getResources()
							.getDimensionPixelSize(R.dimen.avatar_size);
				} else {
					imageSize = getContext().getResources()
							.getDimensionPixelSize(R.dimen.icon_size);
					if (colorPrimary > -1) {
						viewHolder.getImageView().setColorFilter(colorPrimary,
								PorterDuff.Mode.SRC_IN);
					}
				}

				imageMarginEnd = getContext().getResources()
						.getDimensionPixelSize(R.dimen.baseline_content)
						- getContext().getResources().getDimensionPixelSize(
								R.dimen.baseline_start) - imageSize;

				ViewGroup.MarginLayoutParams layoutParams = (ViewGroup.MarginLayoutParams) viewHolder
			...
			if (drawerItem.hasTextPrimary()) {
				viewHolder.getTextViewPrimary().setText(
						drawerItem.getTextPrimary());
				...
				if (drawerItem.hasTextSecondary()
						&& (drawerItem.getTextMode() == DrawerItem.TWO_LINE || drawerItem
								.getTextMode() == DrawerItem.THREE_LINE)) {
					viewHolder.getTextViewSecondary().setText(
							drawerItem.getTextSecondary());
					viewHolder.getTextViewSecondary().setVisibility(
							View.VISIBLE);
	...
	private static class ViewHolder {
		...
		public ViewHolder(View root) {
			...
			mTextViewSecondary = (TextView) root
